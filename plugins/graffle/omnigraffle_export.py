...
def export(source, target, canvasname=None, format=None, debug=False, force=False):
    if format == None:
        format = target.split(".")[-1]
...
def export_one(schema, filename, canvasname, format=None, force=False):
    if format == None:
